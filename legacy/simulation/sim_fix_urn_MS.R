...
delta=qnorm(seq(1/(n_items+1),n_items/(n_items+1),length=n_items),mu_delta,sigma_delta/4)
...
container_item = array(0, dim = c(n_items, n_games, n_reps))
...
for(i in 1:length(student_urn_size)){
  ...
  for(r in 1:n_reps){
    ...
    container[,,r] = matrix(temp[[10]],ncol=n_games)/ student_urn_size[i]
    container_item[,,r] = matrix(temp[[11]],ncol=n_games) / item_urn_size_curr
  }
  result_list[[i]]$mean = apply(container, c(1,2), mean)
  result_list[[i]]$var = apply(container, c(1,2), var)
  result_list[[i]]$true = Theta[,1]
  result_list[[i]]$item_mean = apply(container_item, c(1,2), mean)
}

saveRDS(result_list, "output/MSResultUrn.Rds")